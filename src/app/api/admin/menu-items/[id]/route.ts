...
const schema=z.object({nameEn:z.string().trim().min(2).max(120),nameKm:z.string().trim().min(2).max(120),priceKhr:z.coerce.number().int().min(0),priceUsd:z.coerce.number().min(0),status:z.enum(["active","inactive"]),imageId:z.string().uuid().nullable().optional(),categoryId:z.string().uuid().nullable().optional(),descriptionEn:z.string().optional().or(z.literal("")),descriptionKm:z.string().optional().or(z.literal(""))});
